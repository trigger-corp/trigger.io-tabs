...
static NSMapTable* tabs_modal_map;
...
+ (void)open:(ForgeTask*)task {
	...
	if (tabs_modal_map == nil) {
		tabs_modal_map = [NSMapTable mapTableWithKeyOptions:NSPointerFunctionsStrongMemory valueOptions:NSPointerFunctionsWeakMemory];
	}
	[tabs_modal_map setObject:modalView forKey:task.callid];
}

+ (void)executeJS:(ForgeTask*)task modal:(NSString*)modal script:(NSString*)script {
	[[tabs_modal_map objectForKey:modal] stringByEvaluatingJavaScriptFromString:task string:script];
}

+ (void)close:(ForgeTask*)task modal:(NSString*)modal {
	[[tabs_modal_map objectForKey:modal] close];
	[task success:nil];
}
