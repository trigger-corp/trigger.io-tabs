...
- (BOOL)webView:(UIWebView *)myWebView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
	...
	if ([[thisurl scheme] isEqualToString:@"forge"]) {
		if ([[thisurl absoluteString] isEqualToString:@"forge://go"]) {
			...
			BOOL safe = NO;
			for (NSString *whitelistedPattern in (NSArray*)[[[ForgeApp sharedApp] appConfig] objectForKey:@"trusted_urls"]) {
			...
			[myWebView stringByEvaluatingJavaScriptFromString:@"window.forge._flushing = true;"];
			NSString *jsResult;
			do {
